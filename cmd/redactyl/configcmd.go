	...
	if enable == "" {
		...
		case "minimal":
			enable = strings.Join([]string{
				...
				// Git hosting tokens
				"github-pat", "gitlab-pat", "gitlab-runner-token",
				// Popular developer ecosystem tokens
				"npm-access-token", "pypi-token", "docker-config-auth",
				...
				// Generic high-signal detectors
				"generic-api-key", "private-key", "jwt",
			}, ",")
		case "maximal":
